...
def prepare_training_data(
    ...
    logger.info("Adding opponent defensive EPA...")
    df['opp_pass_def_epa'] = 0.0
    df['opp_rush_def_epa'] = 0.0
    df['team_pace'] = 0.0

    for idx, row in df.iterrows():
        try:
            opp_team = row['opponent_team']
            season = row['season']

            # Calculate defensive EPA for opponent
            def_epa = calculate_team_defensive_epa(
                pbp_df=pbp_df,
                team=opp_team,
                weeks=10,
                season=season
            ...
            df.at[idx, 'opp_pass_def_epa'] = def_epa.get('pass_def_epa', 0.0)
            df.at[idx, 'opp_rush_def_epa'] = def_epa.get('rush_def_epa', 0.0)
        except Exception as e:
            # FIX CRITICAL-3: Log errors instead of silent failure
            logger.warning(f"Failed to calculate defensive EPA for {opp_team} (season {season}): {e}")
            # Keep default 0.0 values already set
