#!/usr/bin/env python3
"""
V19 Fast Training Script

Uses vectorized batch feature extraction with V19 features targeting:
- player_rush_yds: O-line health, box count expected
- player_reception_yds: slot snap %, target share trailing

Target: Complete training in <30 seconds

Usage:
    python scripts/train/train_v19_fast.py
"""
...
def train_market_fast(
    ...
    """
    logger.info(f"\n{'='*60}")
    logger.info(f"Training V19 (Fast): {market}")
    logger.info(f"{'='*60}")
    start = time.time()
...
def main():
    ...
    print("="*80)
    print("V19 FAST TRAINING (Rush/Receiving Yards Improvement)")
    print("="*80)
    print(f"\nMODEL VERSION: V{MODEL_VERSION}")
    print(f"FEATURES: {len(FEATURE_COLS)} columns (V18 + V19)")
    print(f"\nV19 NEW FEATURES:")
    print(f"  - oline_health_score (rushing)")
    print(f"  - box_count_expected (rushing)")
    print(f"  - slot_snap_pct (receiving)")
    print(f"  - target_share_trailing (receiving)")
    ...
    # =========================================================================
    # Summary - Compare V19 vs V18
    # =========================================================================
    print("\n" + "="*80)
    print("V19 MODEL SUMMARY")
    print("="*80)
    ...
    model_data = {
        ...
        'description': f'V{MODEL_VERSION} model with V19 rush/receiving improvements',
        ...
        'v19_features': {
            ...
            'slot_snap_pct': 'Slot route proxy from aDOT',
            'target_share_trailing': '4-week rolling target share',
        }
    }

    # Save paths
    v19_path = PROJECT_ROOT / 'data' / 'models' / f'v{MODEL_VERSION.lower()}_interaction_classifier.joblib'
    joblib.dump(model_data, v19_path)
    logger.info(f"\nSaved to: {v19_path}")
