...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    # Freshness check (REQUIRED - fail if critical data is stale)
    ...
    if not success:
        print(f"\n❌ Pipeline failed: Data freshness check failed")
        print("   Run 'python scripts/fetch/check_data_freshness.py --force' to refresh data")
        sys.exit(1)

    # Model predictions (CRITICAL - longest step)
    success, duration, error = run_sequential_step(
