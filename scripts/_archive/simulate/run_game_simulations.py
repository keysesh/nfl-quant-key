...
def get_week_games(week: int, season: int = None):
    ...
    if not sched_file.exists():
        raise FileNotFoundError(f"NFLverse schedules not found: {sched_file}. Run scripts/fetch/pull_2024_season_data.py first.")
...
def get_team_scoring_average(team: str, season: int = None):
    ...
    NO HARDCODED DEFAULTS - uses actual game results.
    """
    import pandas as pd
    ...
    if not sched_path.exists():
        raise FileNotFoundError(f"NFLverse schedules not found: {sched_path}. Run data/fetch_nflverse_r.R first.")

    sched = pd.read_parquet(sched_path)
    season_games = sched[(sched['season'] == season) & (sched['home_score'].notna())]

    if season_games.empty:
        raise ValueError(f"No completed games found for {season} season. Ensure NFLverse data is up to date.")
    ...
    if all_scores.empty:
        raise ValueError(f"No games found for team {team_abbrev} in {season} season. Check team abbreviation.")
...
def get_team_pace(team: str, season: int = None):
    ...
    NO HARDCODED DEFAULTS - uses actual play counts.
    """
    import pandas as pd
    ...
    if not pbp_path.exists():
        raise FileNotFoundError(f"NFLverse PBP not found: {pbp_path}. Run data/fetch_nflverse_r.R first.")
    ...
    if team_plays.empty:
        raise ValueError(f"No plays found for team {team_abbrev} in {season} season.")
...
def generate_team_projections(home_team: str, away_team: str, season: int = None):
    ...
    NO HARDCODED DEFAULTS - all values come from actual game data.
    """
    # CRITICAL: Use actual team-specific data from NFLverse, not hardcoded league averages
    ...
    # CRITICAL: Use actual team scoring averages, NOT hardcoded 23.0
...
def run_simulations(week: int, season: int = None):
    ...
    for game in games:
        ...
        # NO HARDCODED 130.0 - use actual team pace data
        actual_plays_per_game = projections['home']['pace'] + projections['away']['pace']

        # Create simulation result
        # The loader expects: home_score_median, away_score_median, pace (or plays_per_game)
        # IMPORTANT: Include both full names AND abbreviations for proper team matching
        sim_result = {
            'week': week,
            'season': season,
            'home_team': home_abbrev,  # Use abbreviation for matching
            'away_team': away_abbrev,  # Use abbreviation for matching
            'home_team_full': home,
            'away_team_full': away,
            'home_score_median': projections['home']['projected_points'],
            'away_score_median': projections['away']['projected_points'],
            'plays_per_game': actual_plays_per_game,  # Actual total plays from NFLverse data
            'home_pace': projections['home']['pace'],  # Actual home team plays per game
            'away_pace': projections['away']['pace'],  # Actual away team plays per game
        ...
        filename = f"sim_{season}_{week:02d}_{away_abbrev}_{home_abbrev}.json"
